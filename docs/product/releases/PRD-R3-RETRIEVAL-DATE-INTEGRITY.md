...
Prototype Search and Almanac are partial interaction examples. Almanac is not the required cross-month Timeline, and the prototype query-in-URL behavior must not be carried into the private product.
...
R3 intends to make the archive findable and date-correctable: cross-month Timeline, exact lexical/date/tag/caption retrieval with deliberate history scope, durable Needs Date Review resolution, and atomic redating. Search privacy and restoration of retrieval/date state are release gates.
...
1. The owner scrolls a cross-month chronological Timeline and opens the same Journal Day detail used by Calendar.
...
6. The owner redates a Source Item and sees the old day, new day, Timeline, Calendar, cover, and retrieval indexes update atomically, with affected derived state marked according to its source binding.

## Functional acceptance

- LID-REF-002: Timeline spans month boundaries, is chronological, distinguishes dates and source types, supports pagination or continuation without duplicates/gaps, and opens authoritative Journal Day detail.
...
- LID-SRC-003: redating preserves immutable Original Timestamp and source origin, moves one source reference in one transaction, recalculates both days, cover eligibility, visibility, Timeline order, search/index state, and affected derived-artifact staleness.
...
- LID-REF-006: Timeline, Search, Needs Date Review, redating preview/confirmation/result, empty/loading/error states, and Journal Day transitions meet the required browser, responsive, keyboard, focus, label, zoom, and reduced-motion contract.
...
- Timeline pagination/continuation produces no duplicate, skipped, or unstable item for a fixed data snapshot.
...
R3 design covers cross-month Timeline, chronological grouping, continuation, empty/loading/error states, Search input, result snippets, filters, deliberate history scope, zero results, private-query behavior, Needs Date Review queue/detail, date selection, redating impact preview, confirmation, interruption, success, failure, and affected-day navigation.

The [UX specification](../../design/UX-SPECIFICATION.md) governs the intended experience. [Prototype v5](../../../prototypes/calendar-ui/index-v5.html) provides partial Search and reading-mode direction only. It lacks the required Timeline, complete review/redating flow, durable state, and safe private-query transport.
...
- Timeline continuation order and stable cursor behavior are specified for a fixed archive snapshot.
...
| Timeline integrity | Zero duplicate or skipped fixture across cross-month continuation | Not yet provided |
...
| Accessibility/browser | No blocking issue in Timeline, Search, review, or redating across the required matrix | Not yet provided |
...
- Search privacy, index, Timeline ordering, review-state, and atomic-redating decisions are reviewable.
...
- Design review covers every Timeline, Search, review, redating, degraded, responsive, and accessibility state.
...
- Known queries, deliberate history scope, cross-month Timeline, Needs Date Review, redating, interruption, retry, query-privacy, index-rebuild, backup, restore, and rollback fixtures pass their specified outcomes.
- Old and new Journal Days, Calendar, cover, Timeline, and index agree after redating and after restore.
...
- Timeline or search silently omits, duplicates, or mis-scopes a known live item.
...
| Design review | Timeline, Search, history scope, review, redating, error, responsive, and accessibility review | Not yet provided |
...
| Functional test report | Known-query, scope, Timeline, review, redating, interruption, and retry results | Not yet provided |
