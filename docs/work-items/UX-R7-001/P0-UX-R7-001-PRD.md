...
Design preflight, meaningful-word, safety/failure, persistent label, versions, stale, suppression, and real-photo-cover states.

This artifact narrows the parent release contract to `UX-R7-001`. It does not expand the release, change source precedence, or make a shared parent document task approval.
...
1. **`UX-R7-001-P-001` — Outcome:** the task produces exactly the outcome above and the result is reviewable without implying a later task or release is complete.
2. **`UX-R7-001-P-002` — Boundary:** every mapped requirement is addressed, every deferred requirement stays absent, and no authentic/private/human-only act is inferred from synthetic or public evidence.
3. **`UX-R7-001-P-003` — Evidence:** the task's named acceptance evidence is retrievable, task-bound, independently reviewable, and no broader than the supported claim.

## Required acceptance evidence

Approved task-specific design for UX-R7-001 — Artwork/Version/Suppression Designs links complete normal/empty/loading/error/interruption/destructive states, responsive behavior, accessibility annotations, prototype dispositions, and Designer sign-off.
