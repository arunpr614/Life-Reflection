...
The source scanner must reason over decoded JavaScript tokens/AST and resolved static imports, not a favorable substring count. Its mandatory rejection corpus includes computed or concatenated loader names/specifiers; `process.getBuiltinModule` and computed, destructured, or aliased equivalents; `global`, `globalThis`, or indirect-global paths used to recover a forbidden loader, process primitive, stream, environment, or network API; Unicode escapes or normalization tricks that spell a forbidden identifier or specifier; dynamic `import()`, `require`, `module.createRequire`, or equivalent loader recovery; and every `node:fs` use other than the named `writeFileSync` call that writes the one approved buffer to literal descriptor `3`. It must reject namespace/default/computed `fs` access, aliases that obscure the callee or descriptor, any path operand, reads, stats, realpaths, opens, extra writes, or a descriptor other than literal `3`. Positive fixtures must still admit the exact reviewed imports and fd3 call. Scanner tests bind the decoded source result and full transitive import closure to the raw module hash; simple comments or strings cannot make a forbidden case pass or an allowed case fail.
...
| --- | --- |
| `docs/council/execution/P0-PHASE1-TASK-DEFINITION-OF-READY.md` | Correct the closed post-candidate registry path in line 116 from the nonexistent `docs/council/execution/releases/P0-R0-STAGE-APPROVAL-REGISTRY.json` to `docs/council/execution/P0-R0-STAGE-APPROVAL-REGISTRY.json`; change no lifecycle meaning. |
...
| `tools/P0-test-stage-runner.mjs` | Prove exact production request denial before future-module path access/copy, launcher/child creation, journal, or launch signal; callback denial; scanner evasions; exact canonical vectors; reconstruction/digest mismatch rejection; and unchanged fail-closed runtime behavior. |
...
7. computed loaders, `process.getBuiltinModule`, Unicode-escaped or normalized forbidden names, indirect `global`/`globalThis` capability recovery, and every non-fd3 `node:fs` use are rejected, while only the exact two imports and literal fd3 write pass;
...
11. candidate `C` passes only candidate-safe focused, structural, public-safety, scanner, negative, and adversarial validation over its seven frozen local blobs; after the ratchet is consumed, two complete clean Stage 0 CI passes are byte-stable at exact consume `S`, and two more complete passes are byte-stable at exact post-merge `SM`.
...
2. **Local frozen seed payload `C`.** From exact `M`, construct and review the seven target blobs above in an isolated local worktree. `C` is only the byte-for-byte payload plus its seven raw blob hashes: it is not a commit, branch, ref, PR head, merge candidate, or runnable exact-main tree and is never pushed or merged. It does not change the integrity manifest or registry. Run only candidate-safe focused, structural, scanner, negative, public-safety, and adversarial checks that do not claim manifest consumption or exact-main completeness. Freeze all seven raw hashes only after the bytes, tests, placeholders, module digest, and governed-evidence contract are final.
...
4. candidate-safe structural, public-safety, frozen-scope, no-effect, scanner, negative, and adversarial checks pass over local payload `C`; full Stage 0 CI is not claimed there, and two clean complete passes succeed separately at exact consumed `S` and exact post-merge `SM`;
