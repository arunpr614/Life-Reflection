/*
 * PROTOTYPE ONLY — Life in Days v11 Needs Date Review.
...
const allowedViews = new Set(["calendar", "almanac", "search", "settings", "date-review"]);
...
// deep-link behavior.
const initialView = requestedView === "date-review"
  ? "calendar"
...
const state = {
  ...
  scrollByView: { calendar: 0, almanac: 0, search: 0, settings: 0, "date-review": 0 },
  viewMemory: { calendar: null, almanac: null, search: null, settings: null, "date-review": null },
...
function isFirstUseFixture() {
  return state.readinessFixture.startsWith("first-use/");
}
...
function renderDateReviewDetail() {
  ...
  return `
    <main id="prototype-main" class="date-review-detail-v11" tabindex="-1">
      <button type="button" class="date-review-back-v11" data-action="close-date-review-item">Back to Needs Date Review</button>
      <header class="date-review-header-v11">
        <p class="eyebrow">Management</p>
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v11</strong>
  ...
  return `
    <section class="settings-panel" aria-labelledby="settings-section-heading">
      ...
      <section class="settings-group" aria-labelledby="voicenotes-settings-title">
        <div class="settings-group-heading"><div><p class="eyebrow">Journal capture</p><h3 id="voicenotes-settings-title" tabindex="-1">VoiceNotes</h3></div><span class="settings-state is-neutral">${voiceNotesStatus}</span></div>
        ${settingsRows([
      ...
      <section class="settings-group" aria-labelledby="telegram-settings-title">
        <div class="settings-group-heading"><div><p class="eyebrow">Photo capture</p><h3 id="telegram-settings-title" tabindex="-1">Telegram</h3></div><span class="settings-state is-neutral">${telegramStatus}</span></div>
        ${settingsRows([
...
function renderUnifiedApp() {
  ...
  const monthCollision = state.shell.pendingMonth?.status === "failed" && state.shell.connection !== "connected"
    ...
    : "";
  return `
    <div class="prototype-app unified-v9 unified-v10 unified-v11 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
...
function canonicalRouteUrl() {
  ...
  if (state.view === "date-review") return url;
...
function captureViewMemory(view) {
  ...
  return {
    ...
    focusSelector: view === "date-review" ? "#date-review-title-v11" : logicalFocusSelector,
...
function restoreViewMemory(view, memory) {
  ...
  state.focusAfterRender = view === "date-review" ? "#date-review-title-v11" : memory.focusSelector;
  return true;
}
...
function setView(nextView, { push = true } = {}) {
  ...
  if (savedView) {
    ...
    syncUrl({ push, originAlreadySaved: push });
    render();
    const restoredFocus = nextView === "date-review" ? "#date-review-title-v11" : savedView.focusSelector;
    ...
    toast(nextView === "settings" ? "Settings opened." : nextView === "date-review" ? "Needs Date Review opened." : `${viewLabels[nextView]} view, ${monthLabel(state.month)}.`);
    return;
  }
...
function openReadinessSettings(readinessKey, originControl) {
  const destinations = {
    voicenotes: { section: "integrations", selector: "#voicenotes-settings-title" },
    telegram: { section: "integrations", selector: "#telegram-settings-title" },
...
function setReadinessFixture(fixtureKey) {
  ...
  state.modal = null;
  state.viewMemory = { calendar: null, almanac: null, search: null, settings: null, "date-review": null };
  state.scrollByView = { calendar: 0, almanac: 0, search: 0, settings: 0, "date-review": 0 };
...
function ensureDateReviewDestinationV11(item, date) {
  ...
  state.readinessFixture = "archive/populated";
  state.almanacEmptyArchive = false;
}
...
function handleClick(event) {
  ...
  if (action === "open-date-review") {
    ...
    syncUrl();
    render();
    root.querySelector("#archive-search-input-v9")?.focus();
...
function handlePopState(event) {
  ...
  const resolvedNextView = allowedViews.has(nextView) ? nextView : "calendar";
  ...
  state.focusDate = state.view === "date-review"
    ? state.focusDate
  ...
  const historyFocusTop = dateReviewMoreOrigin ? null : Number.isFinite(snapshot?.focusTop) ? snapshot.focusTop : null;
  ...
  window.history.replaceState({ entryId }, "", canonicalRouteUrl());
  render();
  if (state.view !== "date-review") queueLiveAnnouncementV10(calendarStatusLive, state.monthAnnouncement);
