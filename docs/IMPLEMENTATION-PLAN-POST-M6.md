...
I am not resolving that here, and I am not designing around it either. **The scheduler ADR is the second ticket of M10** — the first *AI-derivation* milestone that genuinely needs it — and every scheduled behaviour in M10, M11, M15 and M17 declares a dependency on it. VoiceNotes reconciliation (M9) is the one exception, and deliberately not an oversight: `LID-VN-005` already requires it to be replay-safe and idempotent, which is exactly the property that makes a bare `cron`-triggered command safe on its own, independent of whatever the M10 ADR eventually decides for provider calls with spend attached. M9 ships before M10 exists, so it couldn't wait for that ADR regardless — it uses a plain systemd timer calling an idempotent reconciliation command, and if the ADR later picks the `job`-table or hybrid option, migrating M9's reconciliation onto it is a small optional follow-up, not a blocker either direction. The ADR decides between the two honest options and one hybrid for everything else:
...
### 7.8 The Hetzner host is shared with two other live products, not zero — and this plan's M6 has not actually been deployed there yet

**Resolution path, added 2026-08-21:** rather than let this risk sit as a paragraph someone has to remember, it now has its own milestone — **`Phase 1.5 — M0.1 — Know the host's real limits before anything else gets built on it`** (milestone 69, issues #316–#324), deliberately numbered ahead of M1 so it's the first thing worked, before any of M1–M19's implementation tickets. It runs two spikes to completion: (1) empirically prove a safe `MemoryHigh`/`MemoryMax`/`CPUWeight` ceiling for Life in Days coexisting with AI Brain and the hackathon-review service, verified by stress-testing a synthetic stand-in against the real host rather than guessing (§7.8's own earlier `MemoryMax=896M` guess, already corrected once in issue #277, gets its actual validation here); (2) survey whether a cheaper or better-fitting hosting option exists at all before committing further build-out to this shared host. Both spikes end in a written decision ticket (#321, #324) the owner can approve.

Verified live on the host 2026-08-21 (SSH `brain@204.168.155.44`, passwordless `sudo`, host `ubuntu-4gb-hel1-1`, 4GB RAM / 38GB disk): two other systemd services are already running — `brain.service` ("AI Brain", Next.js, `127.0.0.1:3000`) and `hackathon-review.service` (`127.0.0.1:4173`) — behind **one** Cloudflare Tunnel (`cloudflared`, tunnel ID `64fb278e-…`) whose single ingress list currently routes `brain.arunp.in`, `brain-staging.arunp.in`, and `hackathon.arunp.in` to those two local ports, falling through to `http_status:404`. There is no second tunnel to create; Life in Days' `life.arunp.in` and `life-hooks.arunp.in` hostnames (`LID-OPS-001`, `LID-OPS-002`) must be **added as two more ingress rules in this same file**, bound to their own local ports, alongside the existing three — not replacing them, not restarting `cloudflared` in a way that drops the other two apps' traffic mid-edit. `/etc/cloudflared/config.yml.pre-hackathon-20260722` and `.pre-d13` show this has already been done safely once before (a dated backup of the previous config kept alongside each new tenant's addition) — M6's deployment tickets (#215/#216) should follow that same pattern, not invent a new one.

Actual resource numbers are tighter than the PRD's thresholds were written against in the abstract, and — this is the part existing plan §8.4 and the PRD's watermark math (`LID-OPS-006`) did not anticipate — **that headroom is shared across three tenants, not reserved for Life in Days.** Measured 2026-08-21: 23GB of 38GB disk free, 3.7GB RAM total with only ~280MB immediately free (the rest is reclaimable page cache, not a hard ceiling, but real under memory pressure). A burst of activity in AI Brain or the hackathon service — logs, a large model download, a traffic spike — can push Life in Days' *own* storage watermarks (§3.13, M14) into a warning or dual-write state that Life in Days did nothing to cause. **Mitigation:** M6's process-supervision ticket (#215) and M13's secure-media-staging ticket (`LID-OPS-005`, "one derivation job at a time on the 4GB host") both need a resource ceiling stated as an absolute number (a memory limit via systemd's `MemoryMax=`, not "whatever's left"), so Life in Days degrades predictably instead of starving or being starved by the other two services; and M14's watermark tickets should measure *host* free space exactly as `LID-OPS-006` specifies (they already do — §3.13's `storage_migration_state` is additive, no change needed there), but the ticket's acceptance criteria should note explicitly that the free-space number reflects all three tenants, not Life in Days alone, so a future reader doesn't mistake a watermark trip for a Life-in-Days-caused storage leak.

`brain.service`'s own unit file, read on the host, is a usable template rather than a cautionary tale — it already does `ProtectSystem=strict`, `ProtectHome=true`, `NoNewPrivileges=true`, a dedicated `brain-data` group, an explicit `ReadWritePaths` allowlist, and an `/opt/brain/current` → versioned-release symlink for atomic deploys, with configuration in `/etc/brain/.env` outside the app directory entirely. It has no `MemoryMax=` set, so there's no existing convention to match there specifically, but everything else is exactly the hardening M6's #215 and M13's `LID-OPS-005`/`LID-OPS-003` already call for — Life in Days' own unit should mirror this shape (`/opt/life-in-days/current`, a dedicated group, `/etc/life-in-days/.env`, the same `ProtectSystem`/`ProtectHome`/`NoNewPrivileges` triad) rather than a from-scratch design, precisely because it is the one pattern already proven safe on this exact host. (One relief while checking this: AI Brain's own description names Ollama as a dependency, which would have been a serious shared-RAM risk on a 4GB box — verified live 2026-08-21 that no Ollama process, binary, or service exists on the host, so that risk does not apply here.)

**Revised, with real host measurements (2026-08-21):** the host has 2 vCPUs. `brain.service` currently uses 239MB RSS; `hackathon-review.service` (a small Python static/auth server) uses 21MB. **Neither service has any cgroup limit set today** — `MemoryMax`, `MemoryHigh`, `MemoryLow`, `MemoryMin`, and `CPUWeight` are all unset/infinity on both, so the only current backstop against any one tenant exhausting the box is the kernel's own OOM killer, which picks a victim by a memory-usage heuristic, not by fault — it could kill AI Brain for a problem Life in Days caused. A single flat `MemoryMax` (the original proposal in ticket #277) is a hard kill: exceeding it terminates the process outright, which turns a transient image-processing spike into a Life-in-Days outage rather than graceful backpressure. The better-fitted pair, standard for cgroup v2, is `MemoryHigh` (a soft throttle point — the kernel reclaims aggressively and slows allocation above it, but does not kill) plus `MemoryMax` set higher, as a true last-resort backstop. Because Life in Days has no code yet, there is no real RSS profile to size these against precisely — ticket #277 should ship an initial pair (`MemoryHigh=512M`, `MemoryMax=1G` is a reasonable starting point given ~3.3GB is genuinely free right now) explicitly labeled as a starting default to be revised from measured behavior once M17's System Health can show real usage, not as a number this plan is treating as final. This ticket deliberately does **not** touch `brain.service` or `hackathon-review.service`'s units — editing another product's already-running production service from a Life in Days ticket is a cross-product change outside this plan's ownership, and the far lower-risk move is to constrain only the new service being added. If the owner later wants the incumbents structurally guaranteed a floor (`MemoryLow`/`MemoryMin` on their own units), that is a decision for whoever maintains those services, not something this plan should do silently on their behalf.

**Correction, verified 2026-08-21 — the claim in an earlier draft of this section was wrong, not just imprecise:** M1–M6's 36 issues (#184–#219) are **all still open** — `gh issue list --milestone ... --state all` shows zero closed issues across milestones 50–55. More fundamentally, **no application code exists on any branch of this repository** — `main`, `plan/implementation`, and `plan/post-m6` all contain only planning documents and reference material; there is no `src/`, no `package.json`, nothing `docs/IMPLEMENTATION-PLAN.md` describes has actually been written. Nothing is deployed on the shared Hetzner host either (no `life.arunp.in` ingress rule, no service, no application directory), which is now the expected consequence of the code not existing, not a separate deployment gap. M7–M19's tickets should **not** be read as building on top of a finished M1–M6 — they cite existing plan §5.1's schema and #185/#196/#199/etc.'s tickets as the intended foundation those tickets describe, not as work already verified running. This is a fact for the owner to confirm, not a decision this document makes on his behalf — but the M7–M19 tickets already filed do assume M1–M6 will eventually exist as described, since redesigning them around "M1–M6 might never be built" would be premature.
...
1. ~~**Hetzner host access timing.**~~ **Resolved 2026-08-21: verified.** SSH `brain@204.168.155.44`, passwordless `sudo ALL:ALL`. See §7.8 for what was found on the live host — two other products already run there, one Cloudflare Tunnel serves all of them, and (corrected from an earlier draft of this document) M1–M6 has not actually been implemented at all yet, on any branch — not just "not deployed."
2. ~~**Provider credentials.**~~ **Resolved 2026-08-21: not yet obtained.** M10's and M11's evaluation gates can be *written* now but cannot be *worked* until credentials arrive — state this explicitly as a blocker on both gate tickets (`LID-AIT-001`, `LID-AIA-001`), not as a silent assumption they're ready.
3. ~~**The $15 evaluation ceiling split.**~~ **Resolved 2026-08-21: even split, $7.50 text / $7.50 artwork.** Both evaluation-gate tickets should state this figure directly rather than leaving the split open.
4. ~~**VoiceNotes test account.**~~ **Resolved 2026-08-21: ready.** `LID-VN-001`'s spike ticket can be worked as soon as it's filed.
5. ~~**Milestone ordering — encryption after capture.**~~ **Resolved 2026-08-21: keep current order** (M13 after M8–M11). The owner accepts the trade in §7.3 — visible progress per milestone, at the cost of real captured content sitting unencrypted on disk from M8 through M12.
6. ~~**B2 and R2 accounts.**~~ **Resolved 2026-08-21: already created.** M14 and M15 tickets can reference real bucket/account setup rather than treating it as a prerequisite step.
7. **New, from §7.8 — shared-host resource ceilings.** M6's process supervision and M13's media-staging tickets need an absolute memory/CPU ceiling for Life in Days so it can't starve or be starved by AI Brain or the hackathon-review service on the same 4GB host. What ceiling? The owner has not set one; a reasonable default (stated in the ticket, not invented as though decided) is worth proposing there rather than left fully open, but the actual number is the owner's to confirm.

Only question 7 remains genuinely open. It does not block filing any ticket — it is exactly the kind of thing a ticket should surface and propose a default for, per the controlling brief's own guidance (§9.1), rather than wait on.
