...
The six-file C1 proposal must be published first through its exact three-projection child—the task-artifact register JSON, roadmap-manifest JSON, and canonical workbook—and a normal two-parent merge. The Markdown release plan must be regenerated in that child and remain byte-identical. Only afterward may a registry-only preparation record be authored as a direct child of the proposal merge and normally merged. Exact-main immutable-history verification must derive the task, proposal bytes, author trailer, reviewer registry, dependency, publication topology, task contract, and all attestations and must replay `evaluateTaskPreparationGateA` successfully.
