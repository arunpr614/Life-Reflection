/*
 * PROTOTYPE ONLY — Life in Days v9 First-use Readiness.
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v9</strong>
  ...
  const content = state.view === "calendar"
    ...
    : state.view === "almanac"
      ...
      : state.view === "search"
        ...
        : renderSettingsView();
  return `
    <div class="prototype-app unified-v9 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
      ${content}
      ${compactNavigation()}
    ...
    <div class="modal-backdrop readiness-disclosure-backdrop-v9" data-action="modal-backdrop">
      <section class="modal-card readiness-disclosure-v9" role="dialog" aria-modal="true" aria-labelledby="readiness-disclosure-title-v9" aria-describedby="readiness-disclosure-intro-v9" tabindex="-1" data-modal-card>
        ...
        ${isRecovery ? `
          ...
          <aside class="settings-footnote" role="note">Version 32 will represent evidence and completion states. V9 cannot prove recovery readiness.</aside>` : `
          ...
          <aside class="settings-footnote" role="note">System Health is the future evidence surface. V9 renders no health metric, snapshot result, or simulated success.</aside>`}
...
function setReadinessFixture(fixtureKey) {
  ...
  state.modal = null;
  state.viewMemory = { calendar: null, almanac: null, search: null, settings: null };
  state.scrollByView = { calendar: 0, almanac: 0, search: 0, settings: 0 };
  ...
  calendarStatusLive.textContent = "";
  requestAnimationFrame(() => { calendarStatusLive.textContent = announcement; });
}
...
function handleClick(event) {
  ...
  if (action === "set-view") {
    ...
    toast(`${label} is a documented management surface outside this v9 Settings prototype.`);
  ...
  else if (action === "nav-placeholder") toast(`${control.dataset.label} is outside this v9 prototype’s review scope.`);
