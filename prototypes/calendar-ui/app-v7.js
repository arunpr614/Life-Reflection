/*
 * PROTOTYPE ONLY — Life in Days v7 Calendar contract completion.
 * All data and mutations are simulated in memory. There are no integrations.
 */

const root = document.querySelector("#prototype-root");
const modalRoot = document.querySelector("#modal-root");
const calendarStatusLive = document.querySelector("#calendar-status-live-v7");
...
const savedThemePreference = window.localStorage.getItem("life-in-days-v7-theme") || "device";
...
const initialSelectedDate = ["calendar", "almanac"].includes(requestedView) && isDateKey(requestedDate) && days[requestedDate] && requestedDate.startsWith(`${initialMonth}-`) ? requestedDate : null;
const state = {
  ...
  almanacCollapsed: initialParams.get("rail") === "collapsed" || window.localStorage.getItem("life-in-days-v7-almanac-collapsed") === "true",
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v7</strong>
    ...
    <main id="prototype-main" class="mosaic-calendar-page ${day ? "has-calendar-selection" : "is-calendar-landing"}" tabindex="-1">
      <div class="mosaic-calendar-layout">
        <section class="mosaic-calendar-column" aria-labelledby="month-heading-mosaic-v7">
          ...
          ${calendarGrid("mosaic", "month-heading-mosaic-v7")}
...
function almanacNavigatorContent(context = "desktop") {
  const headingId = `month-heading-index-${context}-v7`;
  return `
    <div class="almanac-index-content-v7" id="almanac-index-content-${context}-v7">
...
function almanacIndexV7() {
  const collapsed = state.almanacCollapsed;
  return `
    <aside class="almanac-index almanac-index-v7 ${collapsed ? "is-collapsed" : ""}" aria-label="Monthly navigation">
      <button type="button" class="almanac-rail-toggle" data-action="toggle-almanac-rail" aria-controls="almanac-index-content-desktop-v7" aria-expanded="${!collapsed}">
...
function renderAlmanacView() {
  ...
  return `
    ...
    <div class="almanac-shell almanac-shell-v7 ${state.almanacCollapsed ? "is-collapsed" : ""}">
      ${almanacIndexV7()}
...
function renderSearchView() {
  ...
  return `
    <main id="prototype-main" class="search-page-v7" tabindex="-1">
      <header class="search-heading-v7">
      ...
      <form class="archive-search-v7" data-action="search-form" role="search">
        <label for="archive-search-input-v7">Words or exact phrase</label>
        <div><input id="archive-search-input-v7" type="search" value="${html(state.searchQuery)}" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="Enter literal text" /><button class="primary-button" type="submit">Search archive</button></div>
      </form>
      <section class="search-results-v7" aria-live="polite" aria-labelledby="search-results-heading-v7">
        ${!hasQuery ? `
          <div class="search-initial-v7">
            <p class="eyebrow">Search without guesswork</p>
            <h2 id="search-results-heading-v7" tabindex="-1">Only the words you enter</h2>
          ...
          <div class="search-results-heading-v7"><h2 id="search-results-heading-v7" tabindex="-1">${results.length} ${results.length === 1 ? "Journal Day" : "Journal Days"}</h2>${results.length ? '<button type="button" class="text-button" data-action="clear-search">Clear search</button>' : ""}</div>
          ${results.length ? `<div class="search-result-grid-v7">${results.map((day) => {
          const cover = calendarCover(day);
          const counts = dayCounts(day);
          return `<button type="button" class="search-result-card-v7" data-action="open-search-result" data-date="${day.date}" aria-label="${html(`${shortDate(day.date)}: ${day.title}${cover?.kind === "artwork" ? ", AI artwork" : ""}`)}" aria-describedby="search-result-description-${day.date}">
            <span class="search-result-media-v7 ${cover ? "has-media" : "is-paper"}">${cover ? `<img src="${html(cover.src)}" alt="" />` : `<span>${dateParts(day.date).day}</span>`}</span>
            <span class="search-result-copy-v7"><small>${shortDate(day.date)} · ${counts.label}${cover?.kind === "artwork" ? " · AI artwork" : ""}</small><strong>${html(day.title)}</strong><span id="search-result-description-${day.date}">${html(day.summary)}</span><em>${day.tags.map((tag) => html(tag)).join(" · ")}</em></span>
          </button>`;
        }).join("")}</div>` : '<div class="search-empty-v7"><h3>No exact matches</h3><p>Life in Days does not infer alternatives. Try another literal word or clear the search.</p><button type="button" class="secondary-button" data-action="clear-search">Clear search</button></div>'}`}
      </section>
    </main>`;
}
  ...
  const content = state.view === "calendar"
    ...
    : state.view === "almanac"
      ...
      : state.view === "search"
        ...
        : renderSettingsView();
  return `
    <div class="prototype-app unified-v7 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
      ${content}
      ${compactNavigation()}
...
function renderAlmanacDrawer() {
  return `
    <div class="modal-backdrop almanac-drawer-backdrop-v7" data-action="modal-backdrop">
      <aside class="almanac-mobile-drawer-v7" role="dialog" aria-modal="true" aria-labelledby="almanac-drawer-title-v7" tabindex="-1" data-modal-card>
        <header><div><p class="eyebrow">Monthly navigation</p><h2 id="almanac-drawer-title-v7">Find a Journal Day</h2></div><button type="button" class="secondary-button" data-action="close-modal">Close</button></header>
...
function syncUrl({ push = false, selectionOpenedInApp, fullDayOpenedInApp, scrollY } = {}) {
  if (push) {
  ...
  window.history[push ? "pushState" : "replaceState"]({
    ...
    selectionOpenedInApp: selectionOpenedInApp ?? (!push && Boolean(window.history.state?.selectionOpenedInApp)),
    fullDayOpenedInApp: fullDayOpenedInApp ?? (!push && Boolean(window.history.state?.fullDayOpenedInApp)),
...
function setCalendarMonth(nextMonth, { push = true, focusDate = null, focusTarget = "calendar-cell" } = {}) {
  ...
  else if (state.view === "almanac" && state.modal?.type === "almanac-drawer") state.modal.focusSelector = ".almanac-mobile-drawer-v7";
  syncUrl({ push });
  render();
  requestAnimationFrame(() => { calendarStatusLive.textContent = state.monthAnnouncement; });
}

function restoreViewScroll(view, exactScrollY = state.scrollByView[view] || 0) {
  requestAnimationFrame(() => window.scrollTo({ top: exactScrollY, behavior: "auto" }));
}
...
function setView(nextView, { push = true } = {}) {
  ...
  if (nextView === "calendar") {
    ...
    state.focusAfterRender = "#archive-search-input-v7";
  } else {
    state.screen = "month";
...
function closeCalendarSelection() {
  ...
  if (window.history.state?.selectionOpenedInApp) {
...
function handleClick(event) {
  ...
  if (action === "set-view") {
    ...
    toast(`${label} is a documented management surface outside this v7 Settings prototype.`);
  } else if (action === "toggle-theme") {
    state.themePreference = resolvedTheme() === "light" ? "dark" : "light";
    window.localStorage.setItem("life-in-days-v7-theme", state.themePreference);
    ...
    window.localStorage.setItem("life-in-days-v7-almanac-collapsed", String(state.almanacCollapsed));
    ...
    syncUrl({ push: true, fullDayOpenedInApp: true });
    render();
  }
    ...
    if (window.history.state?.fullDayOpenedInApp) {
      window.history.back();
      return;
    }
    state.screen = "month";
    ...
    syncUrl();
    render();
    root.querySelector("#archive-search-input-v7")?.focus();
  ...
  else if (action === "nav-placeholder") toast(`${control.dataset.label} is outside this v7 prototype’s review scope.`);
...
function handleChange(event) {
  ...
  if (control.dataset.action === "set-theme-preference") {
    ...
    window.localStorage.setItem("life-in-days-v7-theme", preference);
...
function handleKeydown(event) {
  if ((event.metaKey || event.ctrlKey) && event.key.toLocaleLowerCase("en-IN") === "k" && !state.modal) {
    ...
    else root.querySelector("#archive-search-input-v7")?.focus();
    return;
  }
  if (event.key === "Enter" && !event.isComposing && event.target.matches("#archive-search-input-v7")) {
    event.preventDefault();
...
function submitSearchValue(value) {
  ...
  state.focusAfterRender = state.searchQuery ? "#search-results-heading-v7" : "#archive-search-input-v7";
  syncUrl();
  render();
}
...
function handleSubmit(event) {
  ...
  if (!form) return;
  event.preventDefault();
  submitSearchValue(form.querySelector("#archive-search-input-v7")?.value);
}
...
function handlePopState(event) {
  ...
  state.selectedDate = ["calendar", "almanac"].includes(state.view) && isDateKey(nextDate) && days[nextDate] && nextDate.startsWith(`${state.month}-`) ? nextDate : null;
  const historyFocusDate = event?.state?.focusDate;
  state.focusDate = datesForMonth(state.month).includes(historyFocusDate)
    ? historyFocusDate
  ...
  // Preserve the live in-memory query while navigating within this open page.
  ...
  state.focusAfterRender = state.view === "search"
    ? "#archive-search-input-v7"
    : state.view === "settings"
      ? "#settings-section-heading"
      : state.view === "calendar" && state.screen === "day"
        ? ".day-detail-header .back-button"
        : state.view === "calendar" && state.selectedDate
        ? ".calendar-selection"
        : state.view === "calendar"
          ? `[data-calendar-date="${state.focusDate}"]`
          : `[data-action="set-view"][data-view="${state.view}"]`;
  syncUrl({ scrollY: Number(event?.state?.scrollY) || 0 });
  render();
  requestAnimationFrame(() => { calendarStatusLive.textContent = state.monthAnnouncement; });
  restoreViewScroll(state.view, Number(event?.state?.scrollY) || 0);
}
...
calendarSelectionSheetQuery.addEventListener("change", () => {
