/*
 * PROTOTYPE ONLY — Life in Days v13 Telegram Capture Companion.
...
const captureStatusLiveV13 = document.querySelector("#capture-status-live-v13");
...
// Pure v13 state machine. DOM, timers, archive mutation, and history stay outside.
...
const days = {
  ...
  "2026-08-13": {
    ...
    photos: [
      {
        id: "p-rain",
        mediaAssetKey: "represented-rain-window-v13",
...
function attachDuplicateCaptureResultV13(fixture, identity) {
  ...
  if (!existingDay || existingPhotoIndex < 0 || existingDay.photos[existingPhotoIndex].mediaAssetKey !== "represented-rain-window-v13") return false;
  const representedReferenceCount = Object.values(days).reduce((count, day) => count + (day.photos || []).filter((photo) => photo.mediaAssetKey === "represented-rain-window-v13").length, 0);
  const referenceId = `capture-duplicate-reference-${representedReferenceCount + 1}-v13`;
...
function attachCaptureResultV13(fixture, identity) {
  ...
  if (["t1", "t4"].includes(fixture.key)) {
    ...
    const photoId = fixture.key === "t4" ? "capture-forwarded-v13" : "capture-receipt-v13";
    ...
    destination.photos = fixture.members.map((member, index) => capturePhotoV13({
      id: `capture-group-member-v13-${index + 1}`,
      src: member.asset,
      alt: member.alt,
...
function nextCaptureSyntheticIdentityV13() {
  let identity = "";
  do {
    identity = `capture-v13-${++captureSyntheticIdentitySequenceV13}`;
  } while (
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v13</strong>
    ...
    <article class="almanac-chapter almanac-chapter-v9 ${state.selectedDate === day.date ? "is-selected" : ""} ${isCurrent ? "is-current" : ""}" id="chapter-${day.date}" data-chapter-date="${day.date}" aria-labelledby="almanac-day-title-${day.date}" tabindex="-1">
      <header class="almanac-chapter-heading-v9">
        <div>
          ...
          <span class="almanac-title-origin-v9">${isRetainedSourceCaption ? "Photo Caption · retained source" : "Generated title · reading aid"}</span>
      ...
      ${chapterMedia(day)}
      ${isRetainedSourceCaption ? "" : `<div class="almanac-reflection-preview-v9">
...
function searchResults(query) {
  ...
  return (state.almanacEmptyArchive ? [] : Object.values(days))
    .filter((day) => {
      const searchableTitle = day.captureOnlyV13 && day.titleStatus === "Source caption" ? "" : day.title;
    ...
    <main id="prototype-main" class="search-page-v9" tabindex="-1">
      ...
      <section class="search-results-v9" aria-live="polite" aria-labelledby="search-results-heading-v9">
        ${!hasQuery ? `
          ...
          const cover = calendarCover(day);
          const counts = dayCounts(day);
          const resultTitle = day.captureOnlyV13 && day.titleStatus === "Source caption" ? "Journal Day" : day.title;
          return `<button type="button" class="search-result-card-v9" data-action="open-search-result" data-date="${day.date}" aria-label="${html(`${shortDate(day.date)}: ${resultTitle}${cover?.kind === "artwork" ? ", AI artwork" : ""}`)}" aria-describedby="search-result-description-${day.date}">
            ...
            <span class="search-result-copy-v9"><small>${shortDate(day.date)} · ${counts.label}${cover?.kind === "artwork" ? " · AI artwork" : ""}</small><strong>${html(resultTitle)}</strong><span id="search-result-description-${day.date}">${html(day.summary)}</span><em>${day.tags.map((tag) => html(tag)).join(" · ")}</em></span>
          </button>`;
...
function settingsIntegrations() {
  ...
  return `
    <section class="settings-panel" aria-labelledby="settings-section-heading">
      ...
      <section class="settings-group" aria-labelledby="capture-source-a-settings-title-v13">
        <div class="settings-group-heading"><div><p class="eyebrow">Journal capture</p><h3 id="capture-source-a-settings-title-v13" tabindex="-1">VoiceNotes</h3></div><span class="settings-state is-neutral">${voiceNotesStatus}</span></div>
        ${settingsRows([
      ...
      <section class="settings-group" aria-labelledby="capture-source-b-settings-title-v13">
        <div class="settings-group-heading"><div><p class="eyebrow">Photo capture</p><h3 id="capture-source-b-settings-title-v13" tabindex="-1">Telegram</h3></div><span class="settings-state is-neutral">${telegramStatus}</span></div>
        ${settingsRows([
...
function captureGuideMarkupV13() {
  ...
  return `
    <details class="telegram-guide" id="capture-guide-v13" ${expanded ? "open" : ""}>
    <summary id="capture-guide-summary-v13">Telegram photo guide</summary>
...
function captureInputMarkupV13() {
  ...
  if (fixture.kind === "guide") {
    return `<section class="telegram-empty" aria-labelledby="capture-selected-title-v13"><h2 id="capture-selected-title-v13" tabindex="-1">Synthetic Telegram message</h2><p>No Telegram activity is being read. Choose a synthetic message to inspect its path.</p></section>`;
  }
  if (fixture.kind === "authorization") {
    return `
      <article class="telegram-input" aria-labelledby="capture-selected-title-v13">
        <header><p class="eyebrow">Selected synthetic request</p><h2 id="capture-selected-title-v13" tabindex="-1">Synthetic Telegram message</h2></header>
...
function capturePathMarkupV13() {
  ...
  if (fixture.kind === "guide" || state.capture.stage === "selected") {
    return `<section class="telegram-conversation" aria-labelledby="capture-path-title-v13"><h2 id="capture-path-title-v13">Simulated Telegram path</h2><p class="telegram-empty">No archive outcome has been represented.</p></section>`;
  }
  ...
  return `
    <section class="telegram-conversation" aria-labelledby="capture-path-title-v13">
      <h2 id="capture-path-title-v13">Simulated Telegram path</h2>
...
function captureDuplicateRelationshipMarkupV13(fixture, { existingDayAction = "capture-view-existing-duplicate-day" } = {}) {
  const referenceCount = Object.values(days).reduce((count, day) => count + (day.photos || []).filter((photo) => photo.mediaAssetKey === "represented-rain-window-v13").length, 0);
  ...
  const crossDayLink = fixture.duplicateScope === "cross-day"
    ...
    : "";
  return `<section class="telegram-media-relationship-v13" aria-labelledby="capture-relationship-title-v13">
    <h3 id="capture-relationship-title-v13">Represented media relationship</h3>
...
function captureDuplicateOutcomeMarkupV13(fixture, terminal) {
  ...
  if (terminal.type === "duplicate-decision" || (terminal.type === "interrupted" && terminal.decisionPreserved)) {
    ...
    return `<section class="telegram-outcome telegram-outcome--review telegram-duplicate-decision-v13" data-state="review" aria-labelledby="capture-outcome-title-v13">
      <h2 id="capture-outcome-title-v13" tabindex="-1">${html(fixture.decisionHeading)}</h2>
      <div class="telegram-outcome-body">
        ...
        <div class="telegram-duplicate-actions-v13">
        ...
        <div class="telegram-actions telegram-existing-day-action-v13">${capturePrivateActionV13("capture-view-existing-duplicate-day", "View 13 August 2026", { disabled: state.shell.connection !== "connected" || permitPending })}</div>
      </div>
    </section>`;
  }
...
function captureConcurrentSecondaryOutcomeMarkupV13(fixture) {
  ...
  return `<section class="telegram-outcome telegram-outcome--accepted telegram-concurrent-secondary-outcome-v13" data-state="accepted" aria-labelledby="capture-secondary-outcome-title-v13">
    <h2 id="capture-secondary-outcome-title-v13" tabindex="-1">Different message result</h2>
    <div class="telegram-outcome-body">
...
function captureOutcomeMarkupV13() {
  ...
  if (!terminal || (fixture.kind === "duplicate" && state.capture.active)) {
    ...
    return `<section class="telegram-outcome" data-state="pending" aria-labelledby="capture-outcome-title-v13"><h2 id="capture-outcome-title-v13">Life in Days outcome</h2><div class="telegram-outcome-body"><p>${html(duplicatePending)}</p></div></section>`;
  }
  ...
  return `
    <main id="prototype-main" class="telegram-capture-page" tabindex="-1">
      ...
      <header class="telegram-capture-header">
        ...
        <h1 id="capture-title-v13" tabindex="-1">Telegram Capture Companion</h1>
...
function duplicateFollowupWouldRequireMultiPriorDatePolicyV13(fixture) {
  return fixture.kind === "duplicate"
    ...
    && Boolean(days[fixture.journalDate]?.photos?.some((photo) => photo.mediaAssetKey === "represented-rain-window-v13"));
}
...
function renderCaptureFixtureConsoleV13() {
  ...
  const duplicateIdentityActions = fixture.kind === "duplicate" && ["duplicate-added", "duplicate-cancelled"].includes(state.capture.terminal?.type)
    ...
    : fixture.kind === "duplicate" && state.capture.active
      ...
      : "";
  return `<section class="telegram-fixture-group-v13 telegram-console" aria-labelledby="capture-console-title-v13"><h3 id="capture-console-title-v13">Telegram Capture Companion · synthetic states</h3><p>Live-memory-only fixtures. No control calls Telegram, a server, AI, or the network.</p><p class="prototype-data-label-v9">Current capture state · ${html(fixture.label || fixture.title || "Guide")} · ${html(state.capture.stage.replaceAll("-", " "))}<span data-capture-replay-proof-v13>${state.capture.replayGuarded ? ` · ${html(state.capture.replayProof)}` : ""}</span></p>${duplicateIdentityActions}${captureConcurrentPermitMarkupV13(fixture)}<div class="telegram-console-grid">
    <fieldset class="telegram-fixture-group"><legend>Scenario</legend><div class="telegram-fixture-controls-v13">${buttons(Object.values(CAPTURE_SCENARIOS_V13).map((item) => [item.key, item.label]), "scenario")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Authorization</legend><div class="telegram-fixture-controls-v13">${buttons(Object.entries(CAPTURE_AUTHORIZATION_FIXTURES_V13), "authorization")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Duplicate handling</legend><div class="telegram-fixture-controls-v13">${buttons(Object.entries(CAPTURE_DUPLICATE_FIXTURES_V13).map(([key, item]) => [key, item.label]), "duplicate")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Media validation</legend><div class="telegram-fixture-controls-v13">${buttons(Object.entries(CAPTURE_MEDIA_FIXTURES_V13).map(([key, item]) => [key, item.label]), "media")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Caption grammar</legend><div class="telegram-fixture-controls-v13">${buttons(Object.entries(CAPTURE_CAPTION_FIXTURES_V13).map(([key, item]) => [key, item.label]), "caption")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Operation</legend><div class="telegram-fixture-controls-v13">${Object.entries(CAPTURE_OPERATION_BRANCHES_V13).map(([key, label]) => `<button type="button" class="text-button" data-action="capture-branch" data-capture-branch="${key}" aria-pressed="${state.capture.branch === key}" ${fixture.kind === "duplicate" ? 'aria-disabled="true"' : ""}>${html(label)}</button>`).join("")}</div></fieldset>
    <fieldset class="telegram-fixture-group"><legend>Duplicate operation</legend><div class="telegram-fixture-controls-v13">${Object.entries(CAPTURE_DUPLICATE_BRANCHES_V13).map(([key, label]) => `<button type="button" class="text-button" data-action="capture-duplicate-branch" data-capture-branch="${key}" aria-pressed="${state.capture.duplicateBranch === key}" ${state.capture.active ? 'aria-disabled="true"' : ""}>${html(label)}</button>`).join("")}</div></fieldset>
...
function selectCaptureFixtureV13(selection, { open = true, push = false, preserveAsyncFocus = false } = {}) {
  ...
  if (open && state.view !== "telegram-capture") {
    ...
    state.screen = "month";
    state.selectedDate = null;
    state.focusAfterRender = preserveAsyncFocus ? null : "#capture-selected-title-v13";
  ...
  const guardedSelector = activeSelectorSurvives && !originOwnedFocus
    ...
    : focusWasUnclaimed || originOwnedFocus || activeSelectorWasRemoved
      ? "#capture-selected-title-v13"
      : null;
  if (!guardedSelector) return;
  if (guardedSelector === "#capture-selected-title-v13") {
    withInstantScroll(() => window.scrollTo({ top: 0, behavior: "auto" }));
  }
...
function deliverDifferentDuplicateMessageV13() {
  ...
  state.capture.surface = "companion";
  state.focusAfterRender = "#capture-selected-title-v13";
  syncUrl();
  render();
...
function replaySettledDuplicateMessageV13() {
  ...
  const proof = root.querySelector("[data-capture-replay-proof-v13]");
...
function attachCaptureGroupProgressV13(fixture, identity, count) {
  ...
  fixture.members.slice(0, count).forEach((member, index) => {
    ...
    const id = `daily-photo-v13-${String.fromCharCode(97 + index)}`;
...
function openCaptureCompanionV13(originControl = null) {
  if (state.view === "telegram-capture") {
    ...
    render();
    withInstantScroll(() => window.scrollTo({ top: 0, behavior: "auto" }));
    const captureTitle = resolveLogicalSelector("#capture-title-v13");
    state.scrollByView["telegram-capture"] = window.scrollY;
    saveCurrentHistorySnapshot({
      scrollY: window.scrollY,
      focusSelector: "#capture-title-v13",
      focusTop: captureTitle?.getBoundingClientRect?.().top ?? null,
    });
    reinforceNavigationFocusV13("#capture-title-v13", "telegram-capture", "companion");
    return;
  }
  ...
  state.selectedDate = null;
  dispatchCaptureV13("SURFACE", { surface: "companion" });
  state.focusAfterRender = "#capture-title-v13";
  syncUrl({ push: true, originAlreadySaved: true, scrollY: 0, focusSelector: state.focusAfterRender, focusTop: null });
  render();
  withInstantScroll(() => window.scrollTo({ top: 0, behavior: "auto" }));
  reinforceNavigationFocusV13("#capture-title-v13", "telegram-capture", "companion");
...
function openCaptureChangeDateV13(control) {
  ...
  state.focusAfterRender = "#capture-change-title-v13";
  syncUrl({ push: true, originAlreadySaved: true, scrollY: 0, focusSelector: state.focusAfterRender, focusTop: null });
  render();
  withInstantScroll(() => window.scrollTo({ top: 0, behavior: "auto" }));
  reinforceNavigationFocusV13("#capture-change-title-v13", "telegram-capture", "change-date");
}
...
function openCaptureDayTargetV13(control, { date, photoId = null, originSelector = null } = {}) {
  ...
  state.focusAfterRender = "#journal-day-title-v13";
  ...
  render();
  withInstantScroll(() => window.scrollTo({ top: 0, behavior: "auto" }));
  reinforceNavigationFocusV13("#journal-day-title-v13", "calendar");
}
...
function viewRelatedMediaDayV13(control) {
  const date = control?.dataset?.relatedDate;
  if (state.view !== "calendar" || state.screen !== "day" || !isDateKey(date) || !days[date]) return;
  const related = days[date].photos.find((photo) => photo.mediaAssetKey === "represented-rain-window-v13");
  if (!related) return;
  openCaptureDayTargetV13(control, { date, photoId: related.id, originSelector: `[data-action="view-related-media-day"][data-related-date="${CSS.escape(date)}"]` });
}
...
function renderUnifiedApp() {
  ...
  const monthCollision = state.shell.pendingMonth?.status === "failed" && state.shell.connection !== "connected"
    ...
    : "";
  return `
    <div class="prototype-app unified-v9 unified-v10 unified-v11 unified-v13 view-${state.view}">
      ${prototypeBanner()}
      ...
      ${currentPhoto && !currentPhotoUnavailable ? `
        <div class="gallery-stage">
          ...
          <div class="gallery-caption">
            ...
            ${currentPhoto.sameMediaDate ? `<div class="same-media-provenance-v13"><strong>Same media as another day</strong><div class="telegram-action-pair"><button type="button" class="text-button" data-action="view-related-media-day" data-related-date="${html(currentPhoto.sameMediaDate)}">View ${html(captureLongDateV13(currentPhoto.sameMediaDate))}</button><span class="telegram-private-link">Private link · authentication required</span></div></div>` : currentPhoto.sharedMedia ? `<div class="same-media-provenance-v13"><strong>Same media as another photo on this day</strong></div>` : ""}
          </div>
        </div>
    ...
    <div class="day-detail day-detail--${mode}">
      <header class="day-detail-header">
        ...
        <div class="day-date-row ${mode === "mosaic" ? "day-date-row--compact" : ""}">
          <div${isCaptureDayHandoffV13 ? ' style="display: block"' : ""}><p class="eyebrow">Journal Day · Asia/Kolkata</p><h1 id="journal-day-title-v13" tabindex="-1">${longDate(day.date)}</h1><p>${counts.label}</p></div>
      ...
      <div class="day-detail-body">
        ${gallery(day)}
        ${day.captureOnlyV13 ? "" : generatedReflection(day, mode)}
        ${sourceJournals(day)}
...
function syncUrl({ push = false, selectionOpenedInApp, fullDayOpenedInApp, captureDayDate, captureDayPhotoId, scrollY, focusSelector, focusTop, originAlreadySaved = false } = {}) {
  ...
  const resolvedCaptureDayDate = isDateKey(captureDayDate)
    ...
    : ["calendar", "almanac"].includes(state.view)
      ...
      && (days[state.selectedDate]?.captureOnlyV13 || previousCaptureDayDate === state.selectedDate)
      ? state.selectedDate
      : null;
...
function revealCaptureFailureOutcomeV13({
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    ...
    const selector = "#capture-outcome-title-v13";
  ...
  return {
    ...
    focusSelector: view === "date-review"
      ...
      : view === "telegram-capture"
        ? logicalFocusSelector || (state.capture.surface === "change-date" ? "#capture-change-title-v13" : "#capture-title-v13")
...
function restoreViewMemory(view, memory) {
  ...
  state.focusAfterRender = view === "date-review"
    ? "#date-review-title-v11"
    : view === "telegram-capture"
      ? memory.focusSelector || (state.capture.surface === "change-date" ? "#capture-change-title-v13" : "#capture-title-v13")
  ...
  if (savedView) {
    ...
    const restoredFocus = nextView === "date-review"
      ...
      : nextView === "telegram-capture"
        ? savedView.focusSelector || "#capture-title-v13"
...
function openReadinessSettings(readinessKey, originControl) {
  const destinations = {
    voicenotes: { section: "integrations", selector: "#capture-source-a-settings-title-v13" },
    telegram: { section: "integrations", selector: "#capture-source-b-settings-title-v13" },
...
async function readJournalFile(file) {
  ...
  try {
    const buffer = await readPrivateJournalFileV13(file);
...
function beginArtworkGeneration(date) {
  ...
  schedulePrivateArchiveTimeoutV13(() => {
    ...
    if (asyncGeneration !== inheritedAsyncGenerationV10 || !currentDay) return;
...
function setShellFixtureV10(fixture) {
  ...
  const sessionFixture = ["shell/session-expired", "shell/session-expired-with-draft"].includes(safeFixture);
  if (sessionFixture) resetSessionHistoryV13();
  if (safeFixture === "shell/correction-interrupted") {
...
function selectorForLogicalFocus(element, { compact = calendarSelectionSheetQuery.matches } = {}) {
  ...
  if (inAlmanacIndex && control?.dataset.action) {
    const fields = ["date", "monthKey", "delta", "view", "section", "photoId", "journalId", "reviewIndex", "pickerDate", "captureGroup", "captureKey", "captureBranch", "fixture", "branch"];
...
function handlePopState(event) {
  ...
  const defaultFocus = state.view === "calendar" && isFirstUseFixture() ? "#first-use-title-v9"
    ...
    : state.view === "settings" ? "#settings-section-heading"
      : state.view === "telegram-capture" ? state.capture.surface === "change-date" ? "#capture-change-title-v13" : "#capture-title-v13"
  ...
  if (resolvedCaptureFocusFallback) focusSelector = "#capture-outcome-title-v13";
