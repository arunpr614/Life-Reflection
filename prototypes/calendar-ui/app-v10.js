/*
 * PROTOTYPE ONLY — Life in Days v10 Resilient Application Shell.
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v10</strong>
...
function renderUnifiedApp() {
  if (["session-expired", "reauth"].includes(state.shell.phase)) {
    return `<div class="prototype-app unified-v9 unified-v10 is-session-gated">${prototypeBanner()}${renderSessionGateV10()}</div>`;
  }
  ...
  const monthCollision = state.shell.pendingMonth?.status === "failed" && state.shell.connection !== "connected"
    ...
    : "";
  return `
    <div class="prototype-app unified-v9 unified-v10 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
...
function handleClick(event) {
  ...
  if (action === "toggle-shell-connection") {
    ...
    else dispatchShell("CONNECTION_INTERRUPT");
  ...
  if (["set-view", "open-settings", "set-settings-section", "open-more", "select-day", "open-day", "open-full-day", "close-day", "adjacent-day", "open-search-result", "previous-month", "next-month", "today", "choose-month"].includes(action)) {
    cancelShellOpsForNavigationV10();
  }
...
function selectorForLogicalFocus(element, { compact = calendarSelectionSheetQuery.matches } = {}) {
  ...
  if (inAlmanacIndex && control?.dataset.action) {
    const fields = ["date", "monthKey", "delta", "view", "section", "photoId", "journalId"];
