...
Translate **Corrections/Conflict/History** into a reversible, bounded implementation or evidence plan for this task only:
...
- **Requirements:** `LID-SCP-004`, `LID-SRC-001`, `LID-SRC-002`, `LID-SRC-004`, `LID-REF-007`, `LID-OPS-011`, `LID-OPS-018`
- **Dependencies:** `UX-R4-001`, `ARCH-R4-001`, `REL-R3-001`
- **Persistent-state / recovery impact:** Adds revision, Correction, conflict, Trash, suppression, and export state; migration must be reversible and backup/restore coverage must pass before release acceptance.
...
1. **`ENG-R4-001-T-001` — Boundary and failure:** invalid, absent, repeated, interrupted, or out-of-order inputs fail safely and leave no partial or falsely successful state.
2. **`ENG-R4-001-T-002` — Recovery:** every persistent shape introduced or changed by this task is inventoried, backed up, restored in a separate empty path, compared, and rolled back or forward-fixed.
3. **`ENG-R4-001-T-003` — Isolation:** privacy, security, resource, dependency, and co-resident failure cannot broaden access, leak sensitive data, or corrupt an accepted earlier release.
