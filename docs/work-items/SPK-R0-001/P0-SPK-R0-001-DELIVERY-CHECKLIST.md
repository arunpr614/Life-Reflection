...
5. **A and AM — exact registry-hash arm.** Only after every exact-candidate review passes may manifest-only `A` directly parent M and arm `next` with exactly one inventory `add` of `P0-R0-STAGE-APPROVAL-REGISTRY.json` to the reviewed preparation-registry SHA-256 while protected `current` remains at cardinality 54. `AM` must be a normal merge with exact parents `[M,A]`, tree equal to A, and only the manifest in its first-parent delta. A and AM themselves grant no preparation permission.
6. **P and PM — preparation record consume.** P directly parents AM, appends exactly `P0-PREP-SPK-R0-001-SYNTHETIC-FOUNDATION`, changes exactly the registry plus manifest, installs the armed hash in `current`, and clears `next`. PM must be a normal merge with exact parents `[AM,P]`, tree equal to P, and the same exact two-path first-parent delta. PR validation binds P and separately fetched AM; runtime derives PM from exact current main and replays the unchanged proof. Direct P-after-M, `PM=[M,P]`, cancellation, stale or interleaved main, wrong hash, extra path, conflict delta, octopus merge, or tree mismatch is incompatible. Only the exact PM replay may yield `Ready to prepare — Gate A`, `preparationAllowed=true`, and `executionAllowed=false`.
...
- [ ] All six C1 blobs are unchanged in C2, M, A, AM, P, and PM.
