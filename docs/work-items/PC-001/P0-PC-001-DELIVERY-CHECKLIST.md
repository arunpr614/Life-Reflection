...
- **Current allowed scope:** planning dossier authoring, exact-candidate Council review, then local/public control hardening only
...
- `P0-OA-002` is not needed to update the existing delivery issue/fields, but it is required before a Project workflow-rule mutation or an unauthorized non-delivery Project item. Neither is planned.
...
- `P0-OA-001` remains mandatory before any private R0 read; `R0-OA-001` and `R0-OA-002` remain due only at their named future gates.

## Two-PR delivery sequence
...
3. Run the complete P0 fixture matrix, deterministic generation, validator, security/public-safety checks, workbook validation, and dry-run projections.
...
6. Publish through a normal PR and merge only after checks and five-seat exit concurrence.
7. Synchronize existing issues/Project from exact clean main using the least expansive dry-run-first mode; do not change workflows.
8. Rebuild/publish Wiki from exact merged main, append the running log, and obtain two quiescent zero-mismatch verifier snapshots.

## Required control surfaces

| Surface | Planning packet | Implementation completion |
| --- | --- | --- |
| Six PC-001 artifacts | Exact shared scope, `in-review`, linked in issue #3 | Final evidence/status language with exact candidate review |
...
- [ ] Six planning artifacts contain identical bounded scope, requirement IDs, test IDs, non-goals, and owner gates.
- [ ] Product PRD passes the project-specific readiness review; Technical, Design, QA, and Delivery plans are complete.
- [ ] All five seats approve one exact planning candidate revision and dossier digest before code edits.
- [ ] Planning PR is merged; issue #3 exposes the merged artifacts; no status/workflow/private mutation occurred.
...
**In review / Hold.** The two-PR path and tracker contract are complete for exact-candidate Council review. All task statuses, issue states, R0 work, private lanes, and Project workflows remain unchanged.
