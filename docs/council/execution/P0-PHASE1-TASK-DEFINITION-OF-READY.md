...
Gate A history is a closed two-PR publication sequence. C1 is a single-parent direct child of the freshly selected exact current-main base, and that base must descend exact minimum Stage 0/R0 anchor `9eb923475421fe566a8d24d89fe09c42f26d2158`. The generic schema permits C1 to change exactly the six canonical proposal artifact paths as regular `100644` `A` or `M` blobs with exact applicable base object IDs, bytes, hashes, safe markers, and no rename/type/mode drift; the first SPK C1 uses the six already existing paths as `M`. C2 is a single-parent direct child of C1 and changes exactly the three generated projections whose bytes encode an in-review-only artifact change: the task-artifact register, JSON roadmap manifest, and canonical workbook. The release-plan Markdown is regenerated and must remain byte-clean because its rendered task-level readiness stays `Incomplete`; an unexpected change to it invalidates C2. The normal proposal merge has exactly two parents, first parent equal to `candidate.baseRevision`, second parent C2, tree equal to C2, and an exact nine-path first-parent delta. The registry-only preparation commit P directly parents that proposal merge; its normal merge PM has the proposal merge as exact first parent and P as exact second parent. Proposal, preparation, implementation, and stage merge boundaries reject octopus merges, post-candidate extras, content-losing merge resolution, side-chain prebuilding, and interleaved first parents. Candidate, proposal merge, P, PM, implementation I/IM, stage S/SM, and current main preserve the same complete task contract; the six artifact blobs and reviewer-registry bytes remain exact at every required boundary. PR validation uses separately fetched `origin/main`; runtime requires the later exact normal-main merge.

`evaluateStageExecutionGateB` wraps the existing exact-candidate evaluator in one immutable stage envelope. It binds exactly one task/stage/scope/action, `execute` or `accept`, the accepted preparation record with independently verified Gate A replay/history, the unchanged full preparation/current/candidate task-contract digest, a unique contiguous per-task sequence and immediately prior predecessor receipt, idempotency key, one passing record per canonical requirement, independent executed QA, rollback plan/snapshot/rehearsal, and trusted append-only stage publication. Implementation I directly descends PM and must already have its own exact two-parent normal-main merge IM before the stage PR; registry-only S directly parents IM, and exact-main runtime requires the later exact two-parent normal-main merge SM. The task-wide cardinality gate remains fail-closed for every direct composite approval; only this wrapper can apply stage-wide cardinality. Its sole non-task-contract branch is code-owned: an ordinary Project Status/issue-state/canonical-status-label stage ending `-DELIVERY-TRANSITION` must bind exact `delivery-control/delivery-status-transition`, module ID `p0.delivery-transition`, the regular-blob reviewed `tools/P0-delivery-transition.mjs` bytes, and all ordinary Gate B evidence. Neither `P0-OA-001` nor `P0-OA-002` is due for that closed delivery-status action. Actual Project workflow configuration or another non-delivery mutation remains outside this branch and, if separately authorized at all, uses `private-execution/project-workflow-mutation` with current private authority and `P0-OA-002`. The task-bound `requestedStageId` exists only in the runtime adapter input and remains forbidden in persisted readiness state. The literal task/global/milestone contracts are not widened.
