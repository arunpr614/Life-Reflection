/*
 * PROTOTYPE ONLY — Life in Days v15 Correction Editor.
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v15</strong>
...
function renderUnifiedApp() {
  ...
  const monthCollision = state.shell.pendingMonth?.status === "failed" && state.shell.connection !== "connected"
    ...
    : "";
  return `
    <div class="prototype-app unified-v9 unified-v10 unified-v11 unified-v14 unified-v15 view-${state.view}">
      ${prototypeBanner()}
    ...
    <section class="day-section sources-section" aria-labelledby="sources-title-${domDate}">
      ...
      <div class="journal-list">
        ${day.journals.map((journal) => {
          ...
          const displayedText = representedCorrection?.text ?? journal.text;
          const sourceLabel = correctionMeta.sourceType;
          return `
          <article class="journal-card correction-source-card-v15${journal.uploadV14 ? " uploaded-journal-card-v14" : ""}" id="correction-card-${html(correctionHandle)}" aria-labelledby="correction-card-title-${html(correctionHandle)}" tabindex="-1">
            <header>
              <div><span class="badge badge-source">${html(sourceLabel)}</span>${representedCorrection ? '<span class="badge badge-correction">Correction displayed · prototype only</span>' : lifecycle.removal ? '<span class="badge badge-prototype-v15">Source Revision displayed</span>' : journal.correction ? '<span class="badge badge-correction">Correction displayed</span>' : ""}<h3 id="correction-card-title-${html(correctionHandle)}" tabindex="-1">${html(journal.title)}</h3></div>
            ...
            <footer><span>Original Timestamp · ${html(journal.timestamp)}</span><span>${html(representedCorrection ? "Correction displayed · source unchanged" : journal.status)}</span></footer>
            ${representedCorrection ? `<aside class="correction-record-v15" aria-label="Displayed Correction metadata">
            ...
            ${!lifecycle.removal && historicalCorrections.length ? `<aside class="correction-retained-v15" aria-label="Retained historical Corrections">
            ...
            <div class="journal-actions">${correctionMeta.eligible
...
function selectorForLogicalFocus(element, { compact = calendarSelectionSheetQuery.matches } = {}) {
  ...
  if (inAlmanacIndex && control?.dataset.action) {
    const fields = ["date", "monthKey", "delta", "view", "section", "photoId", "journalId", "reviewIndex", "pickerDate", "captureGroup", "captureKey", "captureBranch", "uploadFixture", "uploadBranch", "correctionHandle", "correctionKind", "correctionBranch", "callbackKind", "fixture", "branch"];
