/*
 * PROTOTYPE ONLY — Life in Days v6 private in-memory Search state.
...
const savedThemePreference = window.localStorage.getItem("life-in-days-v6-theme") || "device";
...
const state = {
  ...
  almanacCollapsed: initialParams.get("rail") === "collapsed" || window.localStorage.getItem("life-in-days-v6-almanac-collapsed") === "true",
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v6</strong>
  ...
  return `
    <main id="prototype-main" class="mosaic-calendar-page ${day ? "has-calendar-selection" : "is-calendar-landing"}" tabindex="-1">
      <div class="mosaic-calendar-layout">
        <section class="mosaic-calendar-column" aria-labelledby="month-heading-mosaic-v6">
          <section class="mosaic-intro">
            <div><p class="eyebrow">A private month in pictures</p><h1 id="month-heading-mosaic-v6">${monthLabel(state.month)}</h1></div>
          ...
          ${calendarGrid("mosaic", "month-heading-mosaic-v6")}
...
function almanacNavigatorContent(context = "desktop") {
  const headingId = `month-heading-index-${context}-v6`;
  return `
    <div class="almanac-index-content-v6" id="almanac-index-content-${context}-v6">
...
function almanacIndexV6() {
  const collapsed = state.almanacCollapsed;
  return `
    <aside class="almanac-index almanac-index-v6 ${collapsed ? "is-collapsed" : ""}" aria-label="Monthly navigation">
      <button type="button" class="almanac-rail-toggle" data-action="toggle-almanac-rail" aria-controls="almanac-index-content-desktop-v6" aria-expanded="${!collapsed}">
...
function renderAlmanacView() {
  ...
  return `
    ...
    <div class="almanac-shell almanac-shell-v6 ${state.almanacCollapsed ? "is-collapsed" : ""}">
      ${almanacIndexV6()}
...
function renderSearchView() {
  ...
  return `
    <main id="prototype-main" class="search-page-v6" tabindex="-1">
      <header class="search-heading-v6">
      ...
      <form class="archive-search-v6" data-action="search-form" role="search">
        <label for="archive-search-input-v6">Words or exact phrase</label>
        <div><input id="archive-search-input-v6" type="search" value="${html(state.searchQuery)}" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="Enter literal text" /><button class="primary-button" type="submit">Search archive</button></div>
      </form>
      <section class="search-results-v6" aria-live="polite" aria-labelledby="search-results-heading-v6">
        ${!hasQuery ? `
          <div class="search-initial-v6">
            <p class="eyebrow">Search without guesswork</p>
            <h2 id="search-results-heading-v6" tabindex="-1">Only the words you enter</h2>
          ...
          <div class="search-results-heading-v6"><h2 id="search-results-heading-v6" tabindex="-1">${results.length} ${results.length === 1 ? "Journal Day" : "Journal Days"}</h2>${results.length ? '<button type="button" class="text-button" data-action="clear-search">Clear search</button>' : ""}</div>
          ${results.length ? `<div class="search-result-grid-v6">${results.map((day) => {
          const cover = calendarCover(day);
          const counts = dayCounts(day);
          return `<button type="button" class="search-result-card-v6" data-action="open-search-result" data-date="${day.date}" aria-label="${html(`${shortDate(day.date)}: ${day.title}${cover?.kind === "artwork" ? ", AI artwork" : ""}`)}" aria-describedby="search-result-description-${day.date}">
            <span class="search-result-media-v6 ${cover ? "has-media" : "is-paper"}">${cover ? `<img src="${html(cover.src)}" alt="" />` : `<span>${dateParts(day.date).day}</span>`}</span>
            <span class="search-result-copy-v6"><small>${shortDate(day.date)} · ${counts.label}${cover?.kind === "artwork" ? " · AI artwork" : ""}</small><strong>${html(day.title)}</strong><span id="search-result-description-${day.date}">${html(day.summary)}</span><em>${day.tags.map((tag) => html(tag)).join(" · ")}</em></span>
          </button>`;
        }).join("")}</div>` : '<div class="search-empty-v6"><h3>No exact matches</h3><p>Life in Days does not infer alternatives. Try another literal word or clear the search.</p><button type="button" class="secondary-button" data-action="clear-search">Clear search</button></div>'}`}
      </section>
    </main>`;
}
  ...
  const content = state.view === "calendar"
    ...
    : state.view === "almanac"
      ...
      : state.view === "search"
        ...
        : renderSettingsView();
  return `
    <div class="prototype-app unified-v6 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
      ${content}
      ${compactNavigation()}
...
function renderAlmanacDrawer() {
  return `
    <div class="modal-backdrop almanac-drawer-backdrop-v6" data-action="modal-backdrop">
      <aside class="almanac-mobile-drawer-v6" role="dialog" aria-modal="true" aria-labelledby="almanac-drawer-title-v6" tabindex="-1" data-modal-card>
        <header><div><p class="eyebrow">Monthly navigation</p><h2 id="almanac-drawer-title-v6">Find a Journal Day</h2></div><button type="button" class="secondary-button" data-action="close-modal">Close</button></header>
...
function syncUrl({ push = false } = {}) {
  ...
  // Never serialize private search terms. Remove legacy v5 query state on every navigation.
  url.searchParams.delete("q");
...
function setView(nextView, { push = true } = {}) {
  ...
  if (nextView === "calendar") {
    ...
    state.focusAfterRender = "#archive-search-input-v6";
  } else {
    state.screen = "month";
...
function handleClick(event) {
  ...
  if (action === "set-view") {
    ...
    toast(`${label} is a documented management surface outside this v6 Settings prototype.`);
  } else if (action === "toggle-theme") {
    state.themePreference = resolvedTheme() === "light" ? "dark" : "light";
    window.localStorage.setItem("life-in-days-v6-theme", state.themePreference);
    ...
    window.localStorage.setItem("life-in-days-v6-almanac-collapsed", String(state.almanacCollapsed));
    ...
    if (state.modal?.type === "almanac-drawer") state.modal.focusSelector = ".almanac-mobile-drawer-v6";
    syncUrl();
    render();
    ...
    syncUrl();
    render();
    root.querySelector("#archive-search-input-v6")?.focus();
  ...
  else if (action === "nav-placeholder") toast(`${control.dataset.label} is outside this v6 prototype’s review scope.`);
...
function handleChange(event) {
  ...
  if (control.dataset.action === "set-theme-preference") {
    ...
    window.localStorage.setItem("life-in-days-v6-theme", preference);
...
function handleKeydown(event) {
  if ((event.metaKey || event.ctrlKey) && event.key.toLocaleLowerCase("en-IN") === "k" && !state.modal) {
    ...
    else root.querySelector("#archive-search-input-v6")?.focus();
    return;
  }
  if (event.key === "Enter" && !event.isComposing && event.target.matches("#archive-search-input-v6")) {
    event.preventDefault();
...
function submitSearchValue(value) {
  ...
  state.focusAfterRender = state.searchQuery ? "#search-results-heading-v6" : "#archive-search-input-v6";
  syncUrl();
  render();
}
...
function handleSubmit(event) {
  ...
  if (!form) return;
  event.preventDefault();
  submitSearchValue(form.querySelector("#archive-search-input-v6")?.value);
}
