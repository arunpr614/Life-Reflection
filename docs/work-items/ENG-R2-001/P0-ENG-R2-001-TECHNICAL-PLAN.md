...
Translate **Telegram Authorization & Durable Capture** into a reversible, bounded implementation or evidence plan for this task only:
...
- **Requirements:** `LID-TG-001`, `LID-TG-002`, `LID-TG-003`, `LID-TG-004`, `LID-TG-005`, `LID-TG-006`, `LID-OPS-005`, `LID-OPS-011`, `LID-OPS-015`, `LID-OPS-018`
- **Dependencies:** `UX-R2-001`, `ARCH-R2-001`, `REL-R1-001`
- **Persistent-state / recovery impact:** Adds encrypted media, derivative, gallery, duplicate-reference, and review-queue state; migration must be reversible and backup/restore coverage must pass before release acceptance.
...
1. **`ENG-R2-001-T-001` — Boundary and failure:** invalid, absent, repeated, interrupted, or out-of-order inputs fail safely and leave no partial or falsely successful state.
2. **`ENG-R2-001-T-002` — Recovery:** every persistent shape introduced or changed by this task is inventoried, backed up, restored in a separate empty path, compared, and rolled back or forward-fixed.
3. **`ENG-R2-001-T-003` — Isolation:** privacy, security, resource, dependency, and co-resident failure cannot broaden access, leak sensitive data, or corrupt an accepted earlier release.
