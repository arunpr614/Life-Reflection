/*
 * PROTOTYPE ONLY — unified Life in Days v3 experience.
...
const state = {
  ...
  month: initialMonth,
  selectedDate: isDateKey(requestedDate) ? requestedDate : initialMonth === "2026-08" ? "2026-08-13" : `${initialMonth}-01`,
  ...
  almanacCollapsed: initialParams.get("rail") === "collapsed" || window.localStorage.getItem("life-in-days-v3-almanac-collapsed") === "true",
...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v3</strong>
...
function calendarTile(date, mode) {
  ...
  return `
    <button
      ...
      role="gridcell"
      tabindex="${date === state.selectedDate ? "0" : "-1"}"
  ...
  return `
    <main id="prototype-main" class="mosaic-calendar-page" tabindex="-1">
      <section class="mosaic-intro">
        <div><p class="eyebrow">A private month in pictures</p><h1 id="month-heading-mosaic-v3">${monthLabel(state.month)}</h1></div>
      ...
      ${calendarGrid("mosaic", "month-heading-mosaic-v3")}
...
function almanacNavigatorContent(context = "desktop") {
  const headingId = `month-heading-index-${context}-v3`;
  return `
    <div class="almanac-index-content-v3" id="almanac-index-content-${context}-v3">
...
function almanacIndexV3() {
  const collapsed = state.almanacCollapsed;
  return `
    <aside class="almanac-index almanac-index-v3 ${collapsed ? "is-collapsed" : ""}" aria-label="Monthly navigation">
      <button type="button" class="almanac-rail-toggle" data-action="toggle-almanac-rail" aria-controls="almanac-index-content-desktop-v3" aria-expanded="${!collapsed}">
...
function renderAlmanacView() {
  ...
  return `
    ...
    <div class="almanac-shell almanac-shell-v3 ${state.almanacCollapsed ? "is-collapsed" : ""}">
      ${almanacIndexV3()}
...
function renderSearchView() {
  ...
  return `
    <main id="prototype-main" class="search-page-v3" tabindex="-1">
      <header class="search-heading-v3">
      ...
      <form class="archive-search-v3" data-action="search-form" role="search">
        <label for="archive-search-input-v3">Search your archive</label>
        <div><input id="archive-search-input-v3" name="query" type="search" value="${html(state.searchQuery)}" autocomplete="off" placeholder="Try rain, market, or quiet" /><button class="primary-button" type="submit">Search</button></div>
      </form>
      <div class="search-suggestions-v3" aria-label="Suggested searches">
        <span>Try</span>
      ...
      <section class="search-results-v3" aria-live="polite" aria-labelledby="search-results-heading-v3">
        <div class="search-results-heading-v3"><h2 id="search-results-heading-v3">${state.searchQuery ? `${results.length} ${results.length === 1 ? "result" : "results"}` : "Recent Journal Days"}</h2><span>All Journal Days</span></div>
        ${results.length ? `<div class="search-result-grid-v3">${results.map((day) => {
          const cover = calendarCover(day);
          const counts = dayCounts(day);
          return `<button type="button" class="search-result-card-v3" data-action="open-search-result" data-date="${day.date}" aria-label="${html(`${shortDate(day.date)}: ${day.title}`)}" aria-describedby="search-result-description-${day.date}">
            <span class="search-result-media-v3 ${cover ? "has-media" : "is-paper"}">${cover ? `<img src="${html(cover.src)}" alt="" />${cover.kind === "artwork" ? '<span class="badge badge-ai">AI artwork</span>' : ""}` : `<span>${dateParts(day.date).day}</span>`}</span>
            <span class="search-result-copy-v3"><small>${shortDate(day.date)} · ${counts.label}</small><strong>${html(day.title)}</strong><span id="search-result-description-${day.date}">${html(day.summary)}</span><em>${day.tags.map((tag) => html(tag)).join(" · ")}</em></span>
          </button>`;
        }).join("")}</div>` : '<div class="search-empty-v3"><h3>No matching Journal Days</h3><p>Try a broader word or clear the search to see recent days.</p><button type="button" class="secondary-button" data-action="clear-search">Show recent days</button></div>'}
      </section>
    </main>`;
}
...
function renderUnifiedApp() {
  ...
  return `
    <div class="prototype-app unified-v3 view-${state.view}">
      ${prototypeBanner()}
      ${unifiedTopbar()}
      ${content}
      ${compactNavigation()}
...
function renderAlmanacDrawer() {
  return `
    <div class="modal-backdrop almanac-drawer-backdrop-v3" data-action="modal-backdrop">
      <aside class="almanac-mobile-drawer-v3" role="dialog" aria-modal="true" aria-labelledby="almanac-drawer-title-v3" tabindex="-1" data-modal-card>
        <header><div><p class="eyebrow">Monthly navigation</p><h2 id="almanac-drawer-title-v3">Find a Journal Day</h2></div><button type="button" class="secondary-button" data-action="close-modal">Close</button></header>
...
function setView(nextView, { push = true } = {}) {
  ...
  const shouldScrollChapter = nextView === "almanac" && previousView === "calendar" && previousScreen === "day";
  if (nextView === "calendar") {
    ...
    state.focusAfterRender = "#archive-search-input-v3";
  }
...
function handleClick(event) {
  ...
  if (action === "set-view") {
    ...
    window.localStorage.setItem("life-in-days-v3-almanac-collapsed", String(state.almanacCollapsed));
    ...
    if (state.modal?.type === "almanac-drawer") state.modal.focusSelector = ".almanac-mobile-drawer-v3";
    syncUrl();
    render();
    ...
    state.screen = "month";
    if (state.view === "almanac") state.pendingChapterScroll = true;
    syncUrl();
    render();
    ...
    syncUrl();
    render();
    root.querySelector("#archive-search-input-v3")?.focus();
  ...
  else if (action === "nav-placeholder") toast(`${control.dataset.label} is outside this v3 prototype’s review scope.`);
...
function handleKeydown(event) {
  if ((event.metaKey || event.ctrlKey) && event.key.toLocaleLowerCase("en-IN") === "k" && !state.modal) {
    ...
    else root.querySelector("#archive-search-input-v3")?.focus();
    return;
  }
...
function handlePopState() {
  ...
  if (isDateKey(nextDate)) state.selectedDate = nextDate;
  ...
  state.modal = null;
  state.focusAfterRender = state.view === "search" ? "#archive-search-input-v3" : `[data-action="set-view"][data-view="${state.view}"]`;
  render();
