...
**Separately, and worth the owner's attention directly rather than only a ticket note:** M6 (#215–#219) is closed in GitHub as of this writing, but nothing on the live host matches it — no `life.arunp.in` ingress rule, no Life in Days systemd service, no application directory under `/opt` or `/home/brain`. Whatever "done" meant when M6's issues were closed, it was not "deployed to the shared production host." M7–M19 ticket-writing proceeds on the assumption that M6's *code* is finished (per the existing plan and its own closed issues) but that the actual Hetzner deployment step is still outstanding work, not a foundation already in place. This is a fact for the owner to confirm or correct, not a decision this document is making on his behalf.
