...
- **Current state:** v14 `PVA-009 Durable Manual Upload` independently passed and is frozen at implementation/evidence commit `f5e75e3` by the documentation-only freeze record that follows; its Product, Design, Council, Implementation, and QA gates are all **A** and package status is **Complete**. The frontend-prototype portions of `LID-UP-001` through `LID-UP-003` are complete; checksum computation, encrypted storage, durable transactions, persistence, export/restore reproduction, backend idempotency, deployment, and production readiness remain unverified.
