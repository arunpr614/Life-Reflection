...
The staged runtime starts with empty production stage/module/callback/outcome-verifier maps. For a short in-process unit, `executeStageFromExactMain` synchronously captures the exact request and accepts only its code-owned callback/module identity plus `trusted-public-synthetic-no-native-io-v1` and bound SHA-256 capability-review evidence. Static review excludes saved writers, direct/native FDs or streams, subprocesses/workers, background output, and sensitive/private raw material; any such need routes to the serializable lane. The runner holds stage/global locks, durably appends `running`, freshly rechecks predecessor/exact-main Gate B/every deadline, and takes actual start time immediately before invocation. Expired/moved authority durably terminalizes no-mutation without calling the callback; the primordial timer uses only the actual-start remaining window and stays capped at five minutes. Ordinary dynamic console/process interception is a guardrail, not a sandbox. The runner restores bindings, requests `AbortSignal` cancellation, and retains both locks until Promise settlement. A callback that ignores abort and never settles is fail-stuck; a throw, early/invalid receipt, deadline, guardrail-detected stream attempt, or post-action source/authority failure becomes `recovery-required`. For long/process/native-output work, the serializable runner synchronously captures six own identity data descriptors and never rereads caller state, uses closed module/argument identifiers, `shell=false`, fixed working directory/environment, one durable lock, process-tree cancellation, and one append-only receipt. Its reviewed definition-bound exact-main Gate B window is limited to four hours. It checks before spawning the inert launcher, arms settlement/cancellation immediately after spawn and before any await, then records child identity durably in the lock. Only a fresh predecessor/exact-main Gate B/all initial/fresh/current deadline check immediately before `LAUNCH_SIGNAL` can start module code. Denial or expiry terminates the waiting launcher; a no-mutation terminal is appended only after quiescence, and terminal replay starts no second launcher. It waits while retaining the lock and reports terminal cancellation only after no-mutation and no-orphan proof. After possible action in either lane, a readable failed-append event permits release only after regular-file/non-symlink and inode validation, explicit exact-tail file fsync, full hash-chain/same-tail reread, then directory fsync; otherwise the stage lock remains pinned and replay cannot execute a second effect. Neither lane returns raw output.
