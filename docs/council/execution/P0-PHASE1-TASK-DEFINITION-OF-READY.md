...
Gate A and Gate B are separate machine decisions. `evaluateTaskPreparationGateA` accepts only one of the five substantive R0 tasks, one task-bound stage ID and intended scope/action, an exact merged proposal candidate, six approved proposal artifacts, dependency evidence, and five distinct exact-candidate seats. A transition proposal may name the later closed private workflow pair only when its task-bound stage ID ends in `-DELIVERY-TRANSITION`; that still grants preparation only. A pass returns `preparationAllowed=true`, the qualified decision `Ready to prepare — Gate A`, immutable local/public/fictional/synthetic bounds, and always `executionAllowed=false`. It cannot satisfy owner action, private authority, acceptance, status, or runtime gates.

`evaluateStageExecutionGateB` wraps the existing exact-candidate evaluator in one immutable stage envelope. It binds exactly one task/stage/scope/action, `execute` or `accept`, accepted preparation review, a unique contiguous per-task sequence and immediately prior predecessor receipt, idempotency key, one passing record per canonical requirement, independent executed QA, rollback plan/snapshot/rehearsal, and trusted append-only stage publication. The task-wide cardinality gate remains fail-closed for every direct composite approval; only this wrapper can apply stage-wide cardinality. Its sole non-task-contract branch is code-owned: a stage ending `-DELIVERY-TRANSITION` must bind exact `private-execution/project-workflow-mutation`, module ID `p0.delivery-transition`, the regular-blob reviewed `tools/P0-delivery-transition.mjs` bytes, candidate-bound private authority, complete `P0-OA-001` and `P0-OA-002` requirement/record evidence, and all ordinary Gate B evidence. Its task-bound `requestedStageId` exists only in the runtime adapter input and remains forbidden in persisted readiness state. The literal task/global/milestone contracts are not widened.

Execution has no exported in-process callback surface. A reviewed stage must resolve through the serializable runner, whose deadline is at most five minutes and capped by the exact Gate B authority window. The runner accepts only the immutable stage/predecessor/idempotency identity; loads the append-only preparation and stage history from exact main; binds the reviewed definition, module bytes, candidate, seats, rollback, and publication hashes; uses `shell=false`, a fixed working directory, minimal environment, a start-signal child recorded before module code runs, and Node permission containment that denies child/worker creation; and never returns raw output. Exit zero is insufficient: the child must write one closed result/evidence receipt, then exact source/authority plus a closed outcome verifier must pass immediately and at two timed-quiescent boundaries. Every verifier pass is followed by a fresh deadline, exact-main, and Gate B recheck, and the runner performs that complete recheck again immediately before terminal receipt append. Only afterward may the journal append `verified-complete`; all three normalized verification digests are bound into the terminal receipt. Ordinary execution is fail-stuck on an existing lock. Stale-lock termination or reconciliation requires a separate owner-nonce-bound reviewed recovery operation, which remains disabled in Stage 0. Both production action/module maps and the outcome-verifier map are empty. The CLI remains diagnostic. A projected permission boolean is not an authorization token.
...
No owner input is needed to create and review local/public task dossiers. Private-system work remains blocked on `P0-OA-001`. Ordinary exact task contracts still do not permit a workflow mutation. The sole dedicated Gate B delivery-transition branch additionally requires exact-candidate `P0-OA-002`; neither owner action is needed or satisfiable at Gate A. Later authentic-content, credentials/OAuth, provider/privacy/spend, recovery-key/ceremony, final R9, and triggered R10 acts remain just-in-time human gates in the Owner Action Ledger.
