...
Translate **Journal/Source/Encryption Schema** into a reversible, bounded implementation or evidence plan for this task only:
...
- **Requirements:** `LID-SCP-002`, `LID-SCP-003`, `LID-UP-001`, `LID-UP-002`, `LID-UP-003`, `LID-OPS-011`, `LID-OPS-018`
...
1. **`ARCH-R1-001-T-001` — Boundary and failure:** invalid, absent, repeated, interrupted, or out-of-order inputs fail safely and leave no partial or falsely successful state.
2. **`ARCH-R1-001-T-002` — Recovery:** every persistent shape introduced or changed by this task is inventoried, backed up, restored in a separate empty path, compared, and rolled back or forward-fixed.
3. **`ARCH-R1-001-T-003` — Isolation:** privacy, security, resource, dependency, and co-resident failure cannot broaden access, leak sensitive data, or corrupt an accepted earlier release.
