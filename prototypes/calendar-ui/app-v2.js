...
function prototypeBanner() {
  return `
    <div class="prototype-banner" role="note">
      ...
      <strong>Throwaway UI prototype · v2</strong>
  ...
  return `
    <div class="day-preview-content">
      <div class="preview-heading">
        ...
        ${day.attention && day.summaryStatus !== "Stale" ? `<span class="status-chip status-attention">${html(day.attention)}</span>` : ""}
      </div>
      ${previewMedia(day)}
      <section class="preview-reflection" aria-labelledby="preview-reflection-title">
        <div class="preview-reflection-origin"><strong>${html(reflection.label)}</strong>${reflection.isStale ? `<span>Summary update available</span>` : `<span>${html(reflection.detail)}</span>`}</div>
        <h3 id="preview-reflection-title">${html(day.title)}</h3>
        <p>${html(day.summary)}</p>
        <p class="preview-topics" aria-label="Topics">${day.tags.map((tag) => html(tag)).join(" · ")}</p>
      </section>
