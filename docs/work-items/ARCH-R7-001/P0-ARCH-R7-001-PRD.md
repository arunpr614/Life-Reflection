...
Define exact adapter/configuration, preflight, idempotent sweep, artifact lifecycle, provenance, budget reservation, suppression, and restore.

This artifact narrows the parent release contract to `ARCH-R7-001`. It does not expand the release, change source precedence, or make a shared parent document task approval.
...
1. **`ARCH-R7-001-P-001` — Outcome:** the task produces exactly the outcome above and the result is reviewable without implying a later task or release is complete.
2. **`ARCH-R7-001-P-002` — Boundary:** every mapped requirement is addressed, every deferred requirement stays absent, and no authentic/private/human-only act is inferred from synthetic or public evidence.
3. **`ARCH-R7-001-P-003` — Evidence:** the task's named acceptance evidence is retrievable, task-bound, independently reviewable, and no broader than the supported claim.

## Required acceptance evidence

Approved task-specific technical plan for ARCH-R7-001 — Artwork Adapter/Sweep/Budget/Provenance links owned modules, ADRs, threats/data flows, interfaces/data shapes, capacity, migration, restore, rollback, and Architect sign-off.
